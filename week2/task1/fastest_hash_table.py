        ...
        if bucket is None:
            ...
            for item in bucket:
                if item.key == key:
                    item.value = value
                    return False
            bucket.insert(0, Item(key, value, bucket[0]))
        self.item_count += 1
        return True
...
def performance_test():
    hash_table = HashTable()
    x_values = [] # for plot fast
    y_values = [] # for plot fast
...
def save_to_csv(iteration, duration):
    with open('rehash5times-collect.csv', 'a', newline='') as file:
